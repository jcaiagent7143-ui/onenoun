...
**然后它输了。** 把这个例子本身拿去消融
（[examples/discovery/ablate.md](examples/discovery/ablate.md)）：2 胜 3 负。
两个版本给出的框架名字是一样的，但有三个任务裁判更喜欢原版，理由是原版
"更克制、不编造事实"。名字被保住了，围绕这些名字的"要多小心地用它们"没保住。
一次在 diff 里看着极漂亮的 55% 精简，仍然可能是个更差的 Skill——只有消融
实验会告诉你。所以要跑它。
